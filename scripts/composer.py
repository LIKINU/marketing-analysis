...
def render(tier, with_guide=False, depth="标准", stamp=True):
    ...
    for sid, title, lvl, cond in secs:
        ...
        for i, s in enumerate(subs, 1):
            ...
            buf.append("")
    buf.append("")
    buf.append(render_selfcheck(tier, depth, len(secs), n_sub))
    buf.append("")
    return "\n".join(buf)


def render_selfcheck(tier, depth, n_ch, n_sub):
    """《交付前自检表》—— 放在骨架末尾，要求写稿人**原样输出并逐项打勾**。

    为什么放在产物里而不是文档里：文档要人记、prompt 要人补；**产物自带的约束才人人一致**。
    用 `<!-- -->` 注释包住 → md2docx 会剥掉，不会漏进交付稿。
    """
    return f"""<!-- ⛔ 交付前必须原样输出下表并逐项标 ✅/❌ —— 有 ❌ 先修好再说完成；缺表 = 未完成

| 交付前自检（{tier} · {depth} 档） | 判定 |
|---|---|
| 章数＝{n_ch}（缺一章即不合格） |  |
| 小点数＝{n_sub}（缺一个小点即不合格） |  |
...
def check_skeletons(outdir):
    ...
    if missing or bad:
        ...
        return 1
    # ⚠️ 内容级守护：粘贴包必须「自足」——含任务指令与自检表。
    #    否则哪天改回"要用户再补一句 prompt"，判据会立刻叫。（用户明确要求：不该靠额外 prompt）
    if os.path.exists(pp):
        txt = io.open(pp, encoding="utf-8").read()
        need = {"不需要任何补充说明": "自足声明", "硬性要求": "硬性要求段", "交付前自检表": "自检表指引"}
        lack = [v for k, v in need.items() if k not in txt]
        if lack:
            print(f"  [NG  ] 粘贴包不自足，缺：{'、'.join(lack)}（会退化成「要用户补 prompt」）")
            print("判定：粘贴包不自足")
            return 1
        print("  [OK  ] 粘贴包自足（含自足声明／硬性要求／自检表指引）")
    print("  [OK  ] 10 份齐全且与 composer 现算结果一致")
    print("判定：现成骨架可用 ✅")
    return 0
...
def build_paste_pack(path, tier="全量分析", depth="标准"):
    ...
    for ln in skill.splitlines():
        ...
        if grab:
            keep.append(ln)
    rules = "\n".join(keep).strip()
    doc = f"""# 任务：按下面的骨架写一份企业营销与商业模式分析报告

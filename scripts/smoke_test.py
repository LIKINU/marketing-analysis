...
CASES = [
    ...
    ("检索 · 节索引含行号",
     ["kb_find.py", "--sections",
      os.path.join(os.path.dirname(F), "..", "references", "03-方法论操作手册.md"), "--top", "3"],
     0, ["节索引"]),
    ("交付 · 金标准稿可直接交付",
     ["deliver_check.py", os.path.join(F, "喵鲜日记-拆解报告.md"), "--words", "8000"],
     0, ["可直接交付"]),
    ("交付 · 没填完的骨架被判死",
     ["deliver_check.py", os.path.join(os.path.dirname(F), "..", "skeletons",
                                       "骨架-全量分析-标准.md")],
     1, ["不可直接交付", "占位符未填"]),
    ("骨架 · 现成骨架齐全未过期",
     ["composer.py", "--check-skeletons", os.path.join(os.path.dirname(F), "..", "skeletons")],
     0, ["现成骨架可用"]),
    ("骨架 · 目录可从成稿重生成",
     ["composer.py", "--toc", os.path.join(F, "喵鲜日记-拆解报告.md")], 0,
     ["## 目录", "执行摘要"]),
    ("骨架 · 每个小点都带填空规格",
